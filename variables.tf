...
variable "grafana_service_type" {
  description = "Тип Service для Grafana (LoadBalancer, ClusterIP, ...)"
  type        = string
  default     = "LoadBalancer"
}

variable "grafana_admin_password" {
  description = "Пароль адміністратора Grafana. Порожній рядок — чарт сам генерує Secret <release>-grafana"
  type        = string
  sensitive   = true
  default     = ""
}
...
variable "monitoring_storage_class" {
  description = "StorageClass для PVC Prometheus/Grafana/Alertmanager"
  type        = string
  default     = "gp2"
}
